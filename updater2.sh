...
#set -x
cd /home/nym2/
# set vars for version checking and url to download the latest release of nym-mixnode2
...
if [ ! -f nym-mixnode_linux_x86_64 ] || [ "$(./nym-mixnode_linux_x86_64 --version | grep Nym | cut -c 13- )" != "$VERSION" ]
   then
       if systemctl list-units --state=running | grep nym-mixnode2
          ...
          else echo " nym-mixnode2.service is inactive or not existing. Downloading new binaries ..." && pwd
    		curl -L -s "$URL" -o "nym-mixnode_linux_x86_64" --cacert /etc/ssl/certs/ca-certificates.crt && echo "Fetching the latest version" && pwd
	   # Make it executable
   chmod +x ./nym-mixnode_linux_x86_64 && chown nym2:nym2 ./nym-mixnode_linux_x86_64
   fi
else
...
#set -x
cd /home/nym2
#select d in /home/nym2/.nym/mixnodes/* ; do test -n "$d" && break; printf "%b\n\n\n" "${WHITE} >>> Invalid Selection"; done
...
vireward=(VJLCyUN19M3oBBXhNVs3U8EThrgHwGAZHbC1oNCBKyJ1sBz5hdqSHhpnbQhkc3WNPU1WjtZTS1GSLVo4)
printf "%b\n\n\n"
...
sudo -u nym2 -H ./nym-mixnode_linux_x86_64 upgrade --id 'NymMixNode' --incentives-address $vireward --current-version $current_version
}

downloader && sleep 2 && upgrade_nym && sleep 5 && systemctl start nym-mixnode2.service
