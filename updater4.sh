...
#set -x
cd /home/nym4/
# set vars for version checking and url to download the latest release of nym-mixnode4
...
if [ ! -f nym-mixnode_linux_x86_64 ] || [ "$(./nym-mixnode_linux_x86_64 --version | grep Nym | cut -c 13- )" != "$VERSION" ]
   then
       if systemctl list-units --state=running | grep nym-mixnode4
          ...
          else echo " nym-mixnode4.service is inactive or not existing. Downloading new binaries ..." && pwd
    		curl -L -s "$URL" -o "nym-mixnode_linux_x86_64" --cacert /etc/ssl/certs/ca-certificates.crt && echo "Fetching the latest version" && pwd
	   # Make it executable
   chmod +x ./nym-mixnode_linux_x86_64 && chown nym4:nym4 ./nym-mixnode_linux_x86_64
   fi
else
...
#set -x
cd /home/nym4
#select d in /home/nym4/.nym/mixnodes/* ; do test -n "$d" && break; printf "%b\n\n\n" "${WHITE} >>> Invalid Selection"; done
...
sudo -u nym4 -H ./nym-mixnode_linux_x86_64 upgrade --id 'NymMixNode' --incentives-address $vireward --current-version $current_version
}

downloader && sleep 2 && upgrade_nym && sleep 5 && systemctl start nym-mixnode4.service
