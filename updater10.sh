...
#set -x
cd /home/nym10/
# set vars for version checking and url to download the latest release of nym-mixnode10
...
if [ ! -f nym-mixnode_linux_x86_64 ] || [ "$(./nym-mixnode_linux_x86_64 --version | grep Nym | cut -c 13- )" != "$VERSION" ]
   then
       if systemctl list-units --state=running | grep nym-mixnode10
          ...
          else echo " nym-mixnode10.service is inactive or not existing. Downloading new binaries ..." && pwd
    		curl -L -s "$URL" -o "nym-mixnode_linux_x86_64" --cacert /etc/ssl/certs/ca-certificates.crt && echo "Fetching the latest version" && pwd
	   # Make it executable
   chmod +x ./nym-mixnode_linux_x86_64 && chown nym10:nym10 ./nym-mixnode_linux_x86_64
   fi
else
...
#set -x
cd /home/nym10
#select d in /home/nym10/.nym/mixnodes/* ; do test -n "$d" && break; printf "%b\n\n\n" "${WHITE} >>> Invalid Selection"; done
...
vireward=(VJLH8cKAiniy6hesq3jmpfCfTMRyeEQ8UPVPT2T5V31mxd3bFQFSqP7S88mBQD6isJeyahDojBAZYM3Y)
printf "%b\n\n\n"
...
sudo -u nym10 -H ./nym-mixnode_linux_x86_64 upgrade --id 'NymMixNode' --incentives-address $vireward --current-version $current_version
}

downloader && sleep 2 && upgrade_nym && sleep 5 && systemctl start nym-mixnode10.service
