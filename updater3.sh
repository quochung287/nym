...
#set -x
cd /home/nym3/
# set vars for version checking and url to download the latest release of nym-mixnode3
...
if [ ! -f nym-mixnode_linux_x86_64 ] || [ "$(./nym-mixnode_linux_x86_64 --version | grep Nym | cut -c 13- )" != "$VERSION" ]
   then
       if systemctl list-units --state=running | grep nym-mixnode3
          ...
          else echo " nym-mixnode3.service is inactive or not existing. Downloading new binaries ..." && pwd
    		curl -L -s "$URL" -o "nym-mixnode_linux_x86_64" --cacert /etc/ssl/certs/ca-certificates.crt && echo "Fetching the latest version" && pwd
	   # Make it executable
   chmod +x ./nym-mixnode_linux_x86_64 && chown nym3:nym3 ./nym-mixnode_linux_x86_64
   fi
else
...
#set -x
cd /home/nym3
#select d in /home/nym3/.nym/mixnodes/* ; do test -n "$d" && break; printf "%b\n\n\n" "${WHITE} >>> Invalid Selection"; done
...
vireward=(VJLJggUy5DJujx4q6w2ymMn8DfAd7k3LKyUCbxzKnqdBmVyZRgWUFBdM7MfkdmQUfXZVhxNDAR9TeaL2)
printf "%b\n\n\n"
...
sudo -u nym3 -H ./nym-mixnode_linux_x86_64 upgrade --id 'NymMixNode' --incentives-address $vireward --current-version $current_version
}

downloader && sleep 2 && upgrade_nym && sleep 5 && systemctl start nym-mixnode3.service
